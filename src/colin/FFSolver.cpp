...
/**
 * This is one to do at the end
 */
void FF::evaluateStateAndUpdatePlan(const FFEvent & actionToBeApplied, ExtendedMinimalState & state, list<FFEvent> & plan)
{
   ...
   list<ActionSegment> helpfulActions;
    ...
    if (actionToBeApplied.time_spec == VAL::E_AT_START) {
        if (rawDebug) cout << "RAW start\n";
        extraEvent = FFEvent(actionToBeApplied.action, state.startEventQueue.back().minDuration, state.startEventQueue.back().maxDuration);
        eventOneDefined = true;
        ...
        if (!RPGBuilder::getRPGDEs(actionToBeApplied.action->getID()).empty()) { // if it's not a non-temporal action

            const int endStepID = state.getInnerState().planLength - 1;
            const int startStepID = endStepID - 1;
            extraEventTwo = FFEvent(actionToBeApplied.action, startStepID, state.startEventQueue.back().minDuration, state.startEventQueue.back().maxDuration);
            extraEvent.pairWithStep = endStepID;
            eventTwoDefined = true;

            if (!TemporalAnalysis::canSkipToEnd(actionToBeApplied.action->getID())) {
                extraEventTwo.getEffects = false;
            }
...
//    } else if (actionToBeApplied.time_spec == VAL::E_AT_END) {
//        if (rawDebug) cout << "RAW end\n";
//        map<int, list<list<StartEvent>::iterator > >::iterator tsiOld = state.entriesForAction.find(actionToBeApplied.first->getID());
//        assert(tsiOld != state.entriesForAction.end());
//
//        list<StartEvent>::iterator pairWith = tsiOld->second.front();
//        tsiOld->second.pop_front();
...
//        if (!actualJustApplied.empty()) justApplied = &actualJustApplied;
        ...
        extraEvent = FFEvent(actionToBeApplied.divisionID);
        eventOneDefined = true;
        stepID = state.getInnerState().planLength - 1;
    }
