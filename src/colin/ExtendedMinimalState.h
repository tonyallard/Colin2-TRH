...
protected:
	...
	ExtendedMinimalState(const ExtendedMinimalState & e) :
			decorated(new MinimalState(*(e.decorated))), startEventQueue(
					e.startEventQueue), timeStamp(e.timeStamp), stepBeforeTIL(
					e.stepBeforeTIL), tilFanIn(e.tilFanIn), tilComesBefore(
					e.tilComesBefore) {
...
public:
	...
	ExtendedMinimalState(const set<int> & f, const vector<double> & sMin,
			const vector<double> & sMax, const map<int, set<int> > & sa,
			const double & ts, const int & nt, const unsigned int & pl) :
			decorated(new MinimalState(f, sMin, sMax, sa, nt, pl)), timeStamp(
					ts), stepBeforeTIL(-1), tilFanIn(0), hasBeenDominated(false) {
	}

	ExtendedMinimalState() :
			decorated(new MinimalState()), timeStamp(0.0), stepBeforeTIL(-1), tilFanIn(
					0), hasBeenDominated(false) {
	}

	ExtendedMinimalState(const ExtendedMinimalState & e,
			MinimalState * const ms) :
			decorated(ms), startEventQueue(e.startEventQueue), timeStamp(
					e.timeStamp), stepBeforeTIL(e.stepBeforeTIL), tilFanIn(
					e.tilFanIn), tilComesBefore(e.tilComesBefore), hasBeenDominated(
					e.hasBeenDominated) {
	...
	static bool queueEqual(const list<StartEvent> & a,
...
//  }
	...
	ExtendedMinimalState * applyAction(const ActionSegment & a,
			const vector<double> & minTimestamps, double minDur = 0.0,
			double maxDur = 0.0) const;
	void applyActionLocally(const ActionSegment & a,
			const vector<double> & minTimestamps, double minDur = 0.0,
			double maxDur = 0.0);
	ExtendedMinimalState * clone() const;

	inline MinimalState * getInnerStatePtr() const{
		return decorated;
	}
};
}
