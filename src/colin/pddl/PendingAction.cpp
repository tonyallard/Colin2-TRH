...
std::ostream & operator<<(std::ostream & output, const PendingAction & action) {
	...
	const std::list<std::pair<PDDL::Proposition, std::pair<VAL::time_spec, bool> > >::const_iterator condItrEnd =
	...
	//Adds
	std::list<PDDL::Proposition>::const_iterator litItr = action.literalAddEffects.begin();
