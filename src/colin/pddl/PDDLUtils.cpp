...
std::list<PDDL::Proposition> getActionEffects(int actionID,
	...
	if (timeQualifier == VAL::time_spec::E_AT_START) {
		if (positive) {
			...
			effects = Planner::RPGBuilder::getStartPropositionDeletes()[actionID];
		}
...
std::list<PDDL::Literal> getActionConditions(int actionID, VAL::time_spec timeQualifier) {
	...
	//Positive First
	std::list<Inst::Literal*> tmpConditions = Planner::RPGBuilder::getInvariantPropositionalPreconditions()[actionID];
	positiveConditions.insert(positiveConditions.end(), tmpConditions.begin(), tmpConditions.end());
	//Then Negative
	tmpConditions = Planner::RPGBuilder::getInvariantNegativePropositionalPreconditions()[actionID];
	negativeConditions.insert(negativeConditions.end(), tmpConditions.begin(), tmpConditions.end());

	//Depending on time spec (start or end) add required conditions

	if (timeQualifier == VAL::time_spec::E_AT_START) {
		//Positive First
		tmpConditions = Planner::RPGBuilder::getStartPropositionalPreconditions()[actionID];
		positiveConditions.insert(positiveConditions.end(), tmpConditions.begin(), tmpConditions.end());
		//Then Negative
		tmpConditions = Planner::RPGBuilder::getStartNegativePropositionalPreconditions()[actionID];
		negativeConditions.insert(negativeConditions.end(), tmpConditions.begin(), tmpConditions.end());
	} else if (timeQualifier == VAL::time_spec::E_AT_END) {
		//Positive First
		tmpConditions = Planner::RPGBuilder::getEndPropositionalPreconditions()[actionID];
		positiveConditions.insert(positiveConditions.end(), tmpConditions.begin(), tmpConditions.end());
		//Then Negative
		tmpConditions = Planner::RPGBuilder::getEndNegativePropositionalPreconditions()[actionID];
		negativeConditions.insert(negativeConditions.end(), tmpConditions.begin(), tmpConditions.end());
	} else {
		std::cerr << "This case not catered for.";
	}
	...
	std::list<PDDL::Literal> tmpLiterals = PDDL::getLiterals(&positiveConditions, true);
...
std::string getActionName(int actionNum) {
	...
	Inst::instantiatedOp* action = Planner::RPGBuilder::getInstantiatedOp(actionNum);
...
bool supported(const PDDL::Proposition * proposition, std::list<PDDL::Proposition> * propositions) {
	...
	const std::list<PDDL::Proposition>::iterator propItrEnd = propositions->end();
...
std::list<PDDL::Literal> getLiterals(std::list<Inst::Literal*> * propositions, bool positive) {
	std::list<PDDL::Literal> literals;
	std::list<Inst::Literal*>::const_iterator propItr = propositions->begin();
	const std::list<Inst::Literal*>::const_iterator propItrEnd = propositions->end();
...
PDDL::TIL getTIL(Planner::FakeTILAction aTIL,
		double aTimestamp) {
...
PDDL::PendingProposition getPendingProposition(
...
std::list<std::string> getPlanPrefix(std::list<Planner::FFEvent>& plan) {
	...
	for (; eventItr != eventItrEnd; eventItr++) {
		ostringstream name;
		name << (*eventItr).lpTimestamp << ": " << PDDL::getActionName((*eventItr).action->getID());
...
bool isAfter(const Planner::FFEvent * event, const Planner::FFEvent * after, std::list<Planner::FFEvent> * plan) {
	...
	const std::list<Planner::FFEvent>::reverse_iterator eventItrEnd = plan->rend();
	for (; eventItr != eventItrEnd; eventItr++) {
